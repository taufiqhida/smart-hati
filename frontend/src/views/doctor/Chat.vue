        ...
        <div class="chat-container">
          ...
          <div class="chat-sidebar">
            ...
            <div class="chat-contact-list">
              ...
              >
                <div class="chat-contact-avatar">{{ getInitials(contact.name) }}</div>
                <div class="chat-contact-info">
                  <div class="chat-contact-name">{{ contact.name }}</div>
                  <div class="chat-contact-role">{{ getRoleLabel(contact.role) }}</div>
                </div>
                <span v-if="contact.unreadCount > 0" class="nav-item-badge">{{ contact.unreadCount }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="chat-main">
            <template v-if="selectedContact">
              <div style="padding: 1rem; border-bottom: 1px solid var(--border-color); display: flex; align-items: center; gap: 0.75rem;">
                <div class="chat-contact-avatar">{{ getInitials(selectedContact.name) }}</div>
                <div>
              ...
              <div class="chat-messages" ref="messagesContainer">
                <div 
                  ...
                  :key="msg.id"
                  class="chat-message"
                  :class="{ sent: msg.senderId === authStore.user?.id, received: msg.senderId !== authStore.user?.id, emergency: msg.isEmergency }"
                >
                  <div>{{ msg.message }}</div>
                  <div style="font-size: 0.65rem; opacity: 0.7; margin-top: 0.25rem;">{{ formatTime(msg.createdAt) }}</div>
                </div>
              </div>

              <div class="chat-input-container">
                <input v-model="newMessage" @keyup.enter="sendMessage" type="text" class="chat-input" placeholder="Ketik pesan..." />
...
const sendMessage = async () => {
  ...
  try {
    const res = await api.post('/chat/send', { receiverId: selectedContact.value.id, message: newMessage.value })
